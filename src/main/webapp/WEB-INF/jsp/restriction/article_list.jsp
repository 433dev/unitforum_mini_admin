...
<ftt:page>
	<ftt:content>
		<section class="content-header">
			<h1>
				<strong class="game_name">${gameName}</strong> <s:message code="resource.restriction.list.message.postlist" />
			</h1>
		</section>
							...
							<table id="tblArticle" class="table table-striped table-bordered">
								<thead>
									<tr>
										...
										<th><s:message code="resource.restriction.list.post.detail" /></th>
									</tr>
								</thead>
								<tbody>
								</tbody>
...
<script>
	...
	function showInfo(articleIdx){
		var data = {
				articleIdx: articleIdx
			};
		
		$.ajaxWM({
			url: "/ajax/restriction/article/show",
			data: data,
			success: function(result) {
				getList();
	...
	function getList(){
		...
		$.ajaxWM({
			url: "/ajax/restriction/article/list",
			data: data,
			success: function(result) {
				...
				for (var d in result.data) {
					...
					dtArticle.row.add([
						...
						moment(new Date(row.histCreateDt)).format("YYYY-MM-DD HH:mm:ss"),
						row.workUser,
						"<button type=\"button\" onclick=\"showInfo('" + row.articleIdx + "');\" class=\"btn btn-sm btn-warning d-show\"><s:message code="resource.restriction.list.post.restore" /></button>",
						"<button type=\"button\" onclick=\"getComment('" + row.comment + "');\" class=\"btn btn-sm btn-primary d-detail\" value = \"" + row.comment + "\"><s:message code="resource.restriction.list.post.viewreason" /></button>"
					])
					.order([0, "desc"])
					.draw(false);
				}				
